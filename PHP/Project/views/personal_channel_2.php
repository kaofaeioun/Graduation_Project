...
<script type="text/javascript">
		function unbroadcast(){
			var request = new XMLHttpRequest();
		request.open("GET", "unbroadcast.php?id=ericlee");
				...
				<div class="personal_area">
					...
					<div class="track">
						<?php
							...
							$sql2 = "SELECT Track_time FROM Track where Track_ID='$an' && Tracked_ID='ericlee'";
							$result2 = mysqli_query($link,$sql2);
						...
						<script type="text/javascript">
								$( document ).on( "click", "#tracked", function() {
	  								var request = new XMLHttpRequest();
								    request.open("GET", "followcancel.php?Tracked_ID=ericlee&Track_ID=<?php echo $an;?>");
								    request.send();
								...
								$( document ).on( "click", "#notrack", function() {
	  								var request = new XMLHttpRequest();
								    request.open("GET", "follow.php?Tracked_ID=ericlee&Track_ID=<?php echo $an;?>");
								    request.send();
					...
					<div class="personal_info">
						<li><img src="image/watcher.png" original title="目前觀看人數">
							<?php
								...
								$sql="SELECT COUNT(User_ID) FROM UserStatus WHERE Status_Time > '$t' && Status='ericlee'";
								$result=mysqli_query($link,$sql);
						...
						<li><img src="image/like.png" original title="追蹤人數">
							<?PHP
								$sql="SELECT Count(Tracked_ID) From Track WHERE Tracked_ID='ericlee'";
								$result=mysqli_query($link,$sql);
								$row=mysqli_fetch_row($result);
								echo $row[0];
							?>
